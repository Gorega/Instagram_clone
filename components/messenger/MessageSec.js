...
import {setPeopleModal, setShowConversationChat} from "../../features/messengerSlice";
...
export default function MessageSec(){
    ...
    const {showConversationChat,conversation,pending} = useSelector((state)=> state.messenger);
    const socket = useRef();
        ...
        <div className={styles.messageHolder}>
            {pending ? "...loading" : <>
                ...
                <form className={`${styles.sendBox} ${blockedAlert && styles.disabledBox}`} onSubmit={postNewMessageHandler}>
                    ...
                    <input type="text" disabled={blockedAlert && true} placeholder="Message..." value={messageText} onChange={(e)=> setMessageText(e.target.value)} />
